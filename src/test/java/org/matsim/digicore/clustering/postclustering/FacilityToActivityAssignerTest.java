...
package org.matsim.digicore.clustering.postclustering;
 ...
 * @author jwjoubert
 */
public class FacilityToActivityAssignerTest{
	...
	public void testSetup(){
		setupClusters();
		File vehicleFile = new File(utils.getOutputDirectory() + "vehicles.xml.gz");
		Assert.assertTrue("Vehicles file does not exist.", vehicleFile.exists());
		
		File facilityFile = new File(utils.getOutputDirectory() + "facilities.xml.gz");
		...
		fr.putAttributeConverter(Point.class, new HullConverter());
		fr.putAttributeConverter(LineString.class, new HullConverter());
		fr.putAttributeConverter(Polygon.class, new HullConverter());
		...
		Assert.assertEquals("Wrong centroid for f2", new Coord((3.0 + 4.0 + 5.0) / 3.0, (1.0 + 3.0 + 3.0) / 3.0), f2.getCoord());
		Assert.assertEquals("Wrong centroid for f3", new Coord(5.0, 6.0), f3.getCoord());
		
		/* Check facility attributes. */
		Assert.assertNotNull("No 'concaveHull' attribute for facility 1", f1.getAttributes().getAttribute("concaveHull"));
		Assert.assertNotNull("No 'digicoreActivityCount' attribute for facility 1", f1.getAttributes().getAttribute("digicoreActivityCount"));
	}
	...
	public void testBuildFacilityQuadTree(){
		...
		try {
			qt = ccg.buildFacilityQuadTree("dummy.xml");
		...
		try{
			qt = ccg.buildFacilityQuadTree(utils.getOutputDirectory() + "facilities.xml.gz");
		} catch (Exception e) {
			e.printStackTrace();
		...
		/* Test facility attributes. */
		ActivityFacility f1 = qt.getClosest(1.0, 5.0);
		Assert.assertNotNull("No 'concaveHull' attribute for facility 1", f1.getAttributes().getAttribute("concaveHull"));
		Assert.assertNotNull("No 'digicoreActivityCount' attribute for facility 1", f1.getAttributes().getAttribute("digicoreActivityCount"));
	}
	...
	public void testReconstructChains(){
		...
		String inputVehiclesFile = utils.getOutputDirectory() + "vehicles.xml.gz";
		...
		DigicoreVehicles dvs = null;
		try {
			dvs = ccg.reconstructChains(qt, inputVehiclesFile, 1, studyArea);
		} catch (IOException e) {
			Assert.fail("Should not have any exceptions.");
		}
		DigicoreVehicle dv = dvs.getVehicles().get(Id.create("v1", DigicoreVehicle.class));
		Assert.assertNotNull("Vehicle v1 not found.", dv);
		
		/* Check that the correct three activities do have facility Ids. */
		DigicoreChain chain = dv.getChains().get(0);
	...
	private void setupClusters(){
		...
		f1.getAttributes().putAttribute("concaveHull", p1);
		f1.getAttributes().putAttribute("digicoreActivityCount", 4);
		...
		f2.getAttributes().putAttribute("concaveHull", p2);
		f2.getAttributes().putAttribute("digicoreActivityCount", 4);
		...
		f3.getAttributes().putAttribute("concaveHull", p3);
		f3.getAttributes().putAttribute("digicoreActivityCount", 1);
		...
		/* Version 2 vehicles container. */
		DigicoreVehicles dvs = new DigicoreVehicles();
		dvs.addDigicoreVehicle(vehicle);
		new DigicoreVehiclesWriter(dvs).write(utils.getOutputDirectory() + "vehicles.xml.gz");
		
		/* Write facilities. */
		FacilitiesWriter fw = new FacilitiesWriter(facilities);
		
		Map<Class<?>, AttributeConverter<?>> converters = new HashMap<Class<?>, AttributeConverter<?>>();
		...
		fw.write(utils.getOutputDirectory() + "facilities.xml.gz");
	}
